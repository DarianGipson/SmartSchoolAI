...
import { 
  ...
  Target, Award, PlayCircle, Smile, Settings
} from 'lucide-react';
...
const DashboardPage = () => {
  ...
  if (!user) {
    return (
      ...
      >
        <h1 className="text-3xl font-bold text-smartSchool-blue mb-4">Access Denied</h1>
        <p className="text-lg text-gray-600 mb-6">Please log in to access your dashboard.</p>
        <Link to="/login">
          <Button size="lg" className="bg-smartSchool-red hover:bg-smartSchool-red/90 text-white font-semibold py-3 px-8 text-lg rounded-xl">
            Go to Login
          </Button>
        </Link>
      </motion.div>
  ...
  const studentName = user && user.role === 'student' ? user.name : "Parent"; // Simplified for now

  return (
    <motion.div 
      className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12 animate-fade-in" // Added container here
      initial="hidden"
      animate="visible"
    ...
    >
      <motion.section variants={itemVariants}>
        ...
        <p className="text-xl text-center text-gray-600">
          {user && user.role === 'parent' ? "Manage your family's learning journey." : "Ready for a new day of learning adventures?"}
        </p>
      </motion.section>
      ...
      {user && user.role === 'parent' && (
        <motion.section variants={itemVariants} className="text-center p-8 bg-white rounded-2xl shadow-xl">
          ...
          <h2 className="text-3xl font-semibold mb-3">Parent Dashboard Overview</h2>
          <p className="text-lg text-gray-600 mb-6">
            This is your central hub. Manage students, track progress, and access resources.
          </p>
          <Link to="/parent-dashboard">
            <Button size="lg" className="bg-smartSchool-blue hover:bg-smartSchool-blue/90 text-white font-semibold py-3 px-8 text-lg rounded-xl">
              <Settings size={20} className="mr-2" /> Go to Full Parent Management
            </Button>
          </Link>
        </motion.section>
      )}
